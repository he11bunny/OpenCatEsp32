    ...
    { // the gyro reaction switch can be toggled on/off by the 'g' token
      ...
      case IMU_EXCEPTION_KNOCKED:
      {
        if (tQueue->cleared() && skill->period == 1)
        {
          ...
          if (!moduleActivatedQ[0])
          { // serial2 may be used to connect serial2 rather than the RGB ultraconic sensor
      ...
      case IMU_EXCEPTION_OFFDIRECTION:
      {
        ...
        if (tQueue->cleared())
        {
          if (skill->period <= 1 || !strcmp(skill->skillName, "vtF"))
          { // not gait or stepping
...
void reaction()
{ // Reminder:  reaction() is repeatedly called in the "forever" loop() of OpenCatEsp32.ino
  if (newCmdIdx)
  {
    // PTLF("-----");
    lowerToken = tolower(token);
    if (initialBoot)
    { //-1 for marking the boot-up calibration state
      fineAdjustQ = true;
      // updateGyroQ = true;
      gyroBalanceQ = true;
    ...
    case T_REST:
    {
      ...
      { // allow turning off a single joint
      ...
      else
      {
        ...
        { // it supports combining multiple commands at one time
          ...
          if (token == T_SERVO_CALIBRATE)
          {
            gyroBalanceQ = false;
            if (target[0] == DOF)
            { // auto calibrate all body joints using servos' angle feedback
              strcpy(newCmd, "rest");
              loadBySkillName(newCmd);
              shutServos();
              printToAllPorts(T_SERVO_CALIBRATE); // avoid the confirmation token blocked by the loop in autoCalibrate function.
              autoCalibrate();
              break;
            }
...
#ifdef VOICE
              if (newCmdIdx == 2)
              { // only deactivate the voice module via serial port
          ...
          {
            if (inLen == 0)
            { // toggle on/off the bootup melody
              ...
              if (soundState && !buzzerVolume)
              {                   // if i want to unmute but the volume was set to 0
      ...
      else
      {
        ...
        for (int i = 0; i < cmdLen; i += group)
        {
          ...
          { // Write a/d pin value
            ...
            {
              analogWrite((uint8_t)newCmd[i + 1], uint8_t(newCmd[i + 2])); // analog value can go up to 255.
    ...
    {
      // PTH("cmdLen = ", cmdLen);
      if (newCmd[0] != 'U' || (newCmd[0] == 'U' && cmdLen == 1))
      { // when reading the distance from ultrasonic sensor, the cmdLen is 3.
    ...
    case T_SERVO_FEEDBACK: // f: print all angles
      ...
      //
      {
        ...
        if (isDigit(newCmd[0]))
        { // the index of servo to read feedback
    ...
    {                         // it will change the token and affect strcpy(lastCmd, newCmd)
...
#ifdef GYRO_PIN
      if (xyzReal[2] > 0 && (fabs(ypr[1]) > 45 || fabs(ypr[2]) > 45))
      { // wait for imu to update
  ...
  { // Conditionally follow servo feedback to gather servo angles
    if (servoFollow())
    { // don't move the joints if no manual movement is detected
      reAttachAllServos();
      setServoP(P_SOFT);
      workingStiffness = false;
